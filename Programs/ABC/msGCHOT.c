...
//printf("\n");
/*El primer estadístico de resumen a probar será la media de la varianza en el número de repeats entre varios loci */		
...
/*Vamos a contar el número de configuraciones de microsatélites*/
if(rs.conflag == 1){
	...
	*(rs.microflag + k) =0;
	...
	rs.numberconfigurations = secnumber;
		...
		*(rs.microflag + j) = 1;
		...
		//fprintf(stderr,"\nTu número de configuraciones SMM es %i",rs.numberconfigurations);
		//printf("%i\n",rs.numberconfigurations);
		free(rs.microflag);
...
rs.microflag = malloc(secnumber * sizeof(int));
	...
	*(rs.microflag + k) =0;
	...
	//fprintf(stderr,"\nTu número de configuraciones es %i",numberconfigurations);
		for(i=0;i<secnumber;i++){
		if(*(rs.microflag + i) == 0){
		for(j=i+1;j<secnumber;j++){
		rs.configflag =0;
